...
export default function VideoGeneratorApp() {
  ...
  const applyTemplateToSelected = () => {
    ...
    setImages((prev) => prev.map((img) => (img.selected ? { ...img, prompt: template.prompt } : img)))

    // Update template usage count
    setTemplates((prev) => prev.map((t) => (t.id === selectedTemplate ? { ...t, usageCount: t.usageCount + 1 } : t)))
  ...
  const applyBulkPrompt = () => {
    ...
    setImages((prev) => prev.map((img) => (img.selected ? { ...img, prompt: bulkPrompt } : img)))
  ...
  const startGeneration = async () => {
    const selectedImages = images.filter((img) => img.selected && img.prompt.trim())
    ...
    try {
      ...
      const metadata = {
        ...
        merge: false, // Can be made configurable
      }
      ...
      setImages((prev) => prev.map((img) => (img.selected ? { ...img, status: "processing" } : img)))
  ...
  const editWithKontext = async (imageId: string, prompt: string) => {
    ...
    try {
      setImages((prev) => prev.map((img) => (img.id === imageId ? { ...img, status: "processing" } : img)))
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800">
      <div className="container mx-auto p-6">
        <div className="mb-8">
          ...
          <p className="text-slate-600 dark:text-slate-400 mt-2">Transform your images into stunning videos with AI</p>
        </div>
                ...
                <CardContent>
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex items-center gap-2">
                        <Checkbox id="show-selected" checked={showOnlySelected} onCheckedChange={setShowOnlySelected} />
                  ...
                  <ScrollArea className="h-96">
                    ...
                    >
                      {filteredImages.map((image) => (
                        ...
                        >
                          <div
                            className={`relative ${viewMode === "list" ? "w-20 h-20 flex-shrink-0" : "aspect-square"}`}
                          >
                            <img
                            ...
                            <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                              <div className="flex gap-2">
                                <Button size="sm" variant="secondary" onClick={() => toggleImageSelection(image.id)}>
                                  {image.selected ? <Check className="w-4 h-4" /> : <Plus className="w-4 h-4" />}
                                </Button>
                          ...
                          <div className={`${viewMode === "list" ? "flex-1" : "p-3"}`}>
                            ...
                            <Textarea
                              ...
                              onChange={(e) =>
                                setImages((prev) =>
                                  prev.map((img) => (img.id === image.id ? { ...img, prompt: e.target.value } : img)),
                                )
                              }
                              rows={2}
                              className="text-sm"
                            ...
                            {mode === "kontext" && image.prompt && (
                              <Button
                                size="sm"
                                onClick={() => editWithKontext(image.id, image.prompt)}
                            ...
                            {image.outputUrl && (
                              <div className="mt-2">
                                {mode === "seedance" ? (
                                  <video src={image.outputUrl} controls className="w-full rounded" />
                                ) : (
                                  <img
              ...
              <Card>
                ...
                <CardContent>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      <p className="text-sm text-slate-600">{selectedCount} image(s) selected for generation</p>
                      {jobStatus && (
                        <div className="flex items-center gap-2">
          ...
          <TabsContent value="gen4" className="space-y-6">
            <Card>
              ...
              <CardContent className="space-y-6">
                ...
                <div>
                  <Label className="text-base font-medium mb-3 block">Reference Images</Label>
                  <div
                  ...
                  >
                    ...
                    <p className="text-sm text-slate-500">Up to 3 images for style reference</p>
                  </div>
                  <input
                        ...
                        <div className="aspect-square relative">
                          ...
                          <div className="absolute top-2 left-2">
                            <Badge variant="secondary">
                              {index === 0 ? "1st" : index === 1 ? "2nd" : "3rd"} Reference
                            </Badge>
                          </div>
                          <Button
                            size="sm"
                        ...
                        <div className="p-3">
                          <div className="flex flex-wrap gap-1 mb-2">
                            {image.tags.map((tag, tagIndex) => (
                              <Badge
                                ...
                                onClick={() => removeTagFromGen4Image(image.id, tagIndex)}
                              >
                          ...
                          <div className="flex gap-2">
                            <Input
                              ...
                              onKeyPress={(e) => {
                                if (e.key === "Enter") {
                                  addTagToGen4Image(image.id, e.currentTarget.value)
                ...
                {/* Generation Prompt */}
                <div>
                  <Label htmlFor="gen4-prompt" className="text-base font-medium mb-3 block">
                    Generation Prompt
                  </Label>
                  <Textarea
                    id="gen4-prompt"
                ...
                <Button
                  ...
                  disabled={isGen4Generating || !gen4Prompt.trim() || gen4ReferenceImages.length === 0}
            ...
            {gen4Generations.length > 0 && (
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                    <ImageIcon className="w-5 h-5" />
                    Generated Images ({gen4Generations.length})
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <ScrollArea className="h-96">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {gen4Generations.map((generation) => (
                        <div key={generation.id} className="border rounded-lg overflow-hidden">
                          <div className="aspect-square relative">
                            {generation.outputUrl ? (
                              <img
                                src={generation.outputUrl || "/placeholder.svg"}
                                alt="Generated"
                                className="w-full h-full object-cover"
                              />
                            ) : (
                  ...
                  <div>
                    ...
                    >
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="720x1280">Vertical HD</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                  <div>
                    <Label className="text-sm font-medium mb-2 block">Duration: {settings.seedance.duration}s</Label>
                    <Slider
                  ...
                  <div className="flex items-center space-x-2">
                    <Switch
                      ...
                      onCheckedChange={(checked) =>
                        setSettings((prev) => ({
                          ...prev,
                          general: { ...prev.general, showCostEstimates: checked },
                        }))
                      }
                    />
                  ...
                  <div>
                    ...
                    <Slider
                      ...
                      onValueChange={([value]) =>
                        setSettings((prev) => ({
                          ...prev,
                          general: { ...prev.general, maxConcurrentJobs: value },
                        }))
                      }
                      min={1}
                      max={5}
                  ...
                  <CardContent className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                        <p className="text-sm font-medium text-blue-700 dark:text-blue-300">Seedance Videos</p>
                        <p className="text-lg font-bold text-blue-900 dark:text-blue-100">
                          ${(selectedCount * 0.12).toFixed(2)}
                        </p>
                        <p className="text-xs text-blue-600 dark:text-blue-400">{selectedCount} selected × $0.12</p>
                      </div>
                      <div className="p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
                        <p className="text-sm font-medium text-green-700 dark:text-green-300">Kontext Edits</p>
                        <p className="text-lg font-bold text-green-900 dark:text-green-100">
                          ${(selectedCount * (settings.kontext.model === "dev" ? 0.003 : 0.055)).toFixed(3)}
                        </p>
                        <p className="text-xs text-green-600 dark:text-green-400">
                          {selectedCount} selected × ${settings.kontext.model === "dev" ? "0.003" : "0.055"}
                        </p>
                      </div>
                    </div>
                  </CardContent>
